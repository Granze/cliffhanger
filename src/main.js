...
const convert = (i = selectedGrade) => output.innerHTML = grades[i][selectedOutputSystem];

const fillGrades = () => {
  const optTemplate = grades.map((grade, index) => `<option value="${index}">${grade[selectedInputSystem]}</option>`);
